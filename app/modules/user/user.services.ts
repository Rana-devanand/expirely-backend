...
import {
  ...
  tokenFingerprint,
  verifyToken,
} from "../../common/service/passport-jwt.service";
...
export class UserService {
  private async issueSession(user: any, deviceId?: string, deviceName?: string) {
    const authResult = createUserTokens(user);
    const decoded = decodeToken(authResult.refreshToken);
    const { error } = await supabaseAdmin.from("auth_sessions").insert({
      id: authResult.sessionId,
      user_id: user.id,
      refresh_token_hash: hashToken(authResult.refreshToken),
    ...
    console.log(JSON.stringify({
      ...
      tokenFingerprint: tokenFingerprint(authResult.refreshToken),
    }));
    return authResult;
  }

  private async rotateSession(token: string) {
    let decoded: any;
    try {
    ...
    if (decoded.type !== "refresh" || !decoded.sid || !decoded.id) {
      throw new AuthSessionError("REFRESH_TOKEN_INVALID", "Invalid refresh token");
    }
    ...
    if (!session) {
      console.warn(JSON.stringify({
        ...
        outcome: "not_found_or_reused",
      }));
      throw new AuthSessionError("SESSION_REVOKED", "Session is no longer active");
    }
    ...
    const { data: user, error: userError } = await supabaseAdmin
      ...
      .single();
    if (userError || !user) {
      throw new AuthSessionError("SESSION_REVOKED", "User not found");
    }

    const authResult = createUserTokens(user, session.id);
    const nextHash = hashToken(authResult.refreshToken);
    const nextExpiry = decodeToken(authResult.refreshToken).exp;
    const { data: rotated, error: rotateError } = await supabaseAdmin
      .from("auth_sessions")
      .update({
        refresh_token_hash: nextHash,
        last_used_at: new Date().toISOString(),
        expires_at: new Date(nextExpiry * 1000).toISOString(),
      })
      .eq("id", session.id)
      .eq("refresh_token_hash", oldHash)
      .is("revoked_at", null)
    ...
    if (rotateError || !rotated) {
      throw new AuthSessionError("REFRESH_TOKEN_REUSED", "Refresh token was already used");
    }
  ...
  async signUp(userData: any) {
    ...
    const authResult = await this.issueSession(data, userData.deviceId, userData.deviceName);
  ...
  async login(credentials: any) {
    ...
    const authResult = await this.issueSession(user, credentials.deviceId, credentials.deviceName);
  ...
  async refreshToken(token: string) {
    const { authResult } = await this.rotateSession(token);
    return {
  ...
  async me(refreshToken: string) {
    try {
      const { authResult, user } = await this.rotateSession(refreshToken);
  ...
  async socialLogin(payload: {
    ...
    try {
      ...
      const authResult = await this.issueSession(
        user,
        (payload as any).deviceId,
        (payload as any).deviceName,
      );
  ...
  async logout(userId: string, sessionId?: string) {
    if (!sessionId) throw new AuthSessionError("SESSION_INVALID", "Session ID is missing");
